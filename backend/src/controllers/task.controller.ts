...
taskRoutes.get('/', async (c) => {
    const userId = c.get<never>('userId' as never) ?? 'default-user-id';
...
taskRoutes.put('/:id', validator.taskId, validator.updateTask, async (c) => {
    ...
    try {
        const updatedTask = await TaskService.updateTask(id, userId as string, data)
...
taskRoutes.patch('/:id/move', validator.taskId, validator.moveTask, async (c) => {
    const { id } = c.req.valid('param')
    const { columnId } = c.req.valid('json')
    const userId = c.get<never>('userId' as never)

    try {
        const movedTask = await TaskService.moveTask(id, userId as string, columnId)
