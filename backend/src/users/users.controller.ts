...
export class UsersController {
	...
	async getById(@Param() params: UserIdParam, @Res() res: Response) {
		try {
			...
			return errorResponse(res, errorMessage, HttpStatus.INTERNAL_SERVER_ERROR)
		}
	}
	...
	async create(@Body() body: CreateUserDto, @Res() res: Response) {
		try {
			...
			const errorMessage =
				error instanceof Error ? error.message : "Unknown error"
			const status = errorMessage.includes("e-mail já está em uso")
				? HttpStatus.CONFLICT
	...
	async login(@Body() body: LoginUserDto, @Res() res: Response) {
		try {
			const { user, token } = await this.usersService.loginUser(
				body.email,
				body.password,
			)
			...
			const errorMessage =
				error instanceof Error ? error.message : "Unknown error"
			const status = errorMessage.includes("inválidas")
	...
	async update(
		...
		try {
			...
			const errorMessage =
				error instanceof Error ? error.message : "Unknown error"
			return errorResponse(res, errorMessage, HttpStatus.BAD_REQUEST)
		}
	}
	...
	async delete(@Param() params: UserIdParam, @Res() res: Response) {
		try {
			...
			const errorMessage =
				error instanceof Error ? error.message : "Unknown error"
			const status = errorMessage.includes("não encontrado")
				? HttpStatus.NOT_FOUND
				: HttpStatus.BAD_REQUEST
			return errorResponse(res, errorMessage, status)
		}
